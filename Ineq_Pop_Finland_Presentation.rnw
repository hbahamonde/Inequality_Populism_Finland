...
%\framesubtitle{Title}
 ...
 \begin{itemize}
  \item {\bf Argument}: we ``\emph{bring income inequality back},'' and argue that {\color{blue}high income inequality} and {\color{blue}perceived threats from skilled immigration} primarily fuel voter support for the Finns Party (Finland).

  \item {\bf Theoretical perspective}: we introduce ``{\color{blue}prospect theory}'' {\tiny{\color{gray}(Kahneman \& Tversky, 1979)}} to the study of far-right support.

  \item {\bf Data and methods}: We analyze official {\color{blue}census} and {\color{blue}electoral data} using linear {\color{blue}panel data methods} with city fixed effects {\tiny{\color{gray}(Angrist \& Pischke, 2009; Gelman \&  Hill, 2006)}}.


  \item {\bf Findings}:  Voters, influenced by {\color{blue} \emph{past} economic conditions} and driven by the {\color{blue} \emph{fear} of losing} their socio-economic status, support far-right populist parties to aggressively compensate for potential losses. 

  \item {\bf Contribution}: We {\color{blue}\emph{reaffirm} the role of economic inequality} in influencing support for far-right parties in Finland, contrasting Patana's (2020) finding that higher inequality {\bf \emph{decreases}} such support.
...
\item[] {\bf Populist campaigns:}
  ...
  \begin{enumerate}
    \item[\checkmark] When parties {\color{blue}frame their campaigns as losses} (e.g., ``Make America Great \emph{Again}''), voters' loss aversion drives them to support them to avoid a socio-economic decline.  
    \end{enumerate}
...
\begin{frame}[c]{The Finns Party and Its Evolution}
    ...
    \begin{itemize}
       \item The FP emerged in 2011 and has become a major right-wing political force in Finland.
       \item Historically rooted in rural poor interests, the FP's support base now spans other socioeconomic groups, including blue-collar workers.
       \item The party's identity is shaped by socio-cultural issues, particularly opposition to immigration, rather than purely economic considerations.
       \item Economic insecurity and regional disparities play a crucial role in explaining support for the FP, with regional unemployment and social assistance levels correlating with increased support for the party.
    \end{itemize} 
...
\begin{column}[T]{0.45\textwidth}
    \begin{minipage}[c][.6\textheight][c]{\linewidth}
    \begin{itemize}
       \item[${\color{blue}(Y_{it}):}$]<1-6> {\color{blue}Votes for the FP}.

       \item[${\color{red}(X_{it}):}$]<2-6> {\color{red}Gini coefficient} at the city level.

       \item[${\color{cyan}(Z_{it}):}$]<3-6> {\color{cyan}Immigration Data} (city level). Recoded the country of origin's:

        \begin{itemize}
          \item<4-6> Predominantly Muslim.
          \item<5-6> Development level.
...
\begin{column}[T]{0.45\textwidth}
    \begin{minipage}[c][.6\textheight][c]{\linewidth}
        \begin{itemize}
            \item Model 9 interacts the {\color{blue}country of origin} of the immigrant (developed/underdeveloped) with {\color{blue}income inequality}: the FP does better when ``developed immigration'' and immigration are high.

            \item {\bf Our interpretation}: \emph{Since individuals are more focused on preventing a decline, they see the potential losses of skilled immigrants in contexts of high inequality}.
        \end{itemize} 
    \end{minipage}
\end{column}
...
\begin{frame}[c]{Wrapping Up}
  \begin{itemize}
   ...
   \item We think the literature is very messy, with different conflicting explanations. 
   \item What we're trying to do is to revive inequality as one of the most important predictors.
   \item Also, we contribute to the literature by introducing prospect theory to the study of far-right support.
   \item Empirically, we also contribute by disaggregating the type of immigration.
  \end{itemize}
\end{frame}
...
\begin{frame}[c]{Limitations}
  \begin{itemize}
       \item We don't have regional-level data on immigration.
       \item {\bf ``Smoking guns'' problem}: we're trying to improve our identification strategy which might \emph{not} directly match with our loss aversion theory. {\bf Comments on this plz!}
  \end{itemize}
\end{frame}
