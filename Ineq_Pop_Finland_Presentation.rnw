...
\begin{itemize}
    \item {\bf Inequality}: While immigration predictors are positive and significant, {\color{blue}income inequality trumps} all of them.

    \begin{itemize}
      \item In fact, {\color{blue}loss aversion} (lagged inequality) is the {\bf strongest predictor}.
    \end{itemize}
    
    \item {\bf Cultural backlash theories}: Muslim immigration is a stronger predictor than total migration.
