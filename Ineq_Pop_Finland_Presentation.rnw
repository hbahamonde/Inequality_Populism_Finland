...
Aki Koivula \inst{1} \\ \vspace{3mm} \scriptsize{\color{gray}Authors in alphabetical order. All contributed equally to this paper.}
}
...
\begin{frame}[c]{A Very \emph{Unmotivating} Motivation}
    ...
    \begin{itemize}
           \item Democracy's status:
              \begin{itemize}
                \item Not in trouble. {\scriptsize{\color{gray}(Voeten, 2016).}}
                \item In trouble. {\scriptsize{\color{gray}(Mounk \& Foa, 2016; Mudde, 2004; Coff\'e et al., 2007).}}
               \end{itemize}

            \item Far-right populism drivers:
               \begin{itemize}
                \item Cultural reasons. {\scriptsize{\color{gray}(Veugelers \& Chiarini, 2002).}}
                \item Psychological factors. {\scriptsize{\color{gray}(Cohen \& Smith, 2016).}}
                \item Identity reasons. {\scriptsize{\color{gray}(Sniderman et al., 2004; Oesch, 2008).}}
               \end{itemize}

            \item Inequality and populism:

               \begin{itemize}
                 \item High inequality. {\scriptsize{\color{gray}(Han, 2016).}}
                 \item Low inequality. {\scriptsize{\color{gray}(Patana, 2020).}}
                 \item And finally, some even  think that ``it's not the economy, stupid!'' {\scriptsize{\color{gray} (Mudde, 2007).}}
               \end{itemize}
