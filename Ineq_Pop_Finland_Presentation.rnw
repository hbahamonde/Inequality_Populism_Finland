...
\begin{column}[T]{0.45\textwidth}
    \begin{minipage}[c][.6\textheight][c]{\linewidth}
                 ...
                 \includegraphics[scale=0.45, center]{/Users/hectorbahamonde/research/Inequality_Populism_Finland/finns_historical_presentation.pdf}
...
\begin{column}[T]{0.45\textwidth}
    ...
    \begin{itemize}
        ...
        \item We include other {\color{cyan}controls} (immigration) and city fixed effects.
...
\begin{column}[T]{0.45\textwidth}
    \begin{minipage}[c][.6\textheight][c]{\linewidth}
     ...
     \begin{equation*}
     {\color{blue}Y}_{it} = \alpha + \beta {\color{red}X}_{it} + \gamma {\color{cyan}Z}_{it} + \lambda_i + \delta_t + \epsilon_{it}
     \end{equation*}
...
\lambda_i & : \text{City fixed effects} \\
  ...
  \begin{itemize}
    \item We hypothesized (and pre-registered) that:
      \begin{enumerate}
          ...
          \item[H2] This effect would be \emph{stronger} in Presidential systems \\ {\tiny(because of its zero-sum power sharing structure, {\bf losses are more catastrophic})}.
      \end{enumerate}
